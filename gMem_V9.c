...
//修改: ( 邓国祖) 20181112   --V9(在V8的基础上，将前后指向增加倍率，提升管理总空间，原来64K,现在64*particle_size(起点256K)，
								    速度上应该有所减弱，还示测试 
******************************************************************************/
#ifndef NULL
#define	NULL				(0)
...
//-------------------分配最小单元---------------------------------
static unsigned short uAppRamParticleSize,uFlagGuideSize;
//----------------异常功能信息显示-----------
...
{
	{
		DfFlagGuideRAM *pNext=(DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
		//------释放节点空间---------------------------
		if((char*)pNext  != pAppRamGuideEnd)
		{
			...
			{//------删除链表(向后)------------
				...
				pNext=(DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
				if((char*)pNext != pAppRamGuideEnd) 
					pNext->uLast =pGuideRAM->uNext;
			}
		}
	}
	if(pGuideRAM->uLast != NULL)
	{
		DfFlagGuideRAM *pLast=(DfFlagGuideRAM *)((char*)pGuideRAM  - (pGuideRAM->uLast*uAppRamParticleSize));
		...
		{//------删除链表(向前)------------
			...
			pGuideRAM=(DfFlagGuideRAM *)((char*)pLast + (pLast->uNext*uAppRamParticleSize));
			if((char*)pGuideRAM != pAppRamGuideEnd)
				pGuideRAM->uLast = pLast->uNext;
			pGuideRAM=pLast;
		}
	}
...
void gMemSplitSpace(DfFlagGuideRAM *pGuideRAM,unsigned silen)
{
	...
	pNew=(DfFlagGuideRAM *)((char*)pGuideRAM + (silen*uAppRamParticleSize));
	pNew->uLast=silen;
	pNew->uNext=((char*)pNext-(char*)pNew)/uAppRamParticleSize;
	if(((char*)pNext) != pAppRamGuideEnd)
	{
		if(pNext->pNfree)
		{
			...
			pNext=(DfFlagGuideRAM *)((char*)pNew + (pNew->uNext*uAppRamParticleSize));
			if((char*)pNext != pAppRamGuideEnd)
...
int gMemAllocInit(void* mem_addr,unsigned mem_size,unsigned particle_size)
{
	...
	if(mem_size > (0xffff*particle_size))
	{	
		pAppRamGuideEnd=NULL;
		pRamQuick=NULL;
	...
	//------------最小单元------------------
	uAppRamParticleSize=4;
	uFlagGuideSize=sizeof(DfFlagGuideRAM)/4;
	{
		unsigned bit3,i,Max;
	...
	uAppRamParticleSize = particle_size;
	uFlagGuideSize=sizeof(DfFlagGuideRAM)/uAppRamParticleSize;
	return 0;
}
...
void *gMalloc(unsigned silen)
{
	...
	//------申请长度整成整数倍(避免指针偏移)--------------
	silen =(sizeof(DfFlagGuideRAM)+silen+uAppRamParticleSize-1)/uAppRamParticleSize; 
	//-----最快的入口-------
	pGuideRAM=pRamQuick;
	pLastfree=NULL;
	while((char*)pGuideRAM != pAppRamGuideEnd)
	{
		if(pGuideRAM->uNext >= silen)
		{
			...
			//--------检查是否可以再分割--------------
			if(pGuideRAM->uNext > (silen+uFlagGuideSize))
			{//---多于的空间，用于增加一个数据空间结点--------
				DfFlagGuideRAM *pNext,*pNew;
				pNext=(DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
				pGuideRAM->uNext=silen;
...
void *gPralloc(unsigned sPreLen,unsigned *OutSize)
{
	...
	{
		//-申请长度整成4整数倍(避免指针偏移)-
		sPreLen =(sizeof(DfFlagGuideRAM)+sPreLen+uAppRamParticleSize-1)/uAppRamParticleSize; 
		while((char*)pGuideRAM != pAppRamGuideEnd)
		{
			if(pGuideRAM->uNext >= sPreLen)
			{
				...
				pGuideRAM->pNfree=NULL; //空闲指针为空(非空闲)表示占用。
				//-------检查余量------------
				if(pGuideRAM->uNext > (sPreLen+uFlagGuideSize))
				{//---多于的空间，用于增加一个数据空间结点--------
					DfFlagGuideRAM *pNext,*pNew;
					pNext=(DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
					pGuideRAM->uNext=sPreLen;
					pNew=(DfFlagGuideRAM *)((char*)pGuideRAM + (sPreLen*uAppRamParticleSize));
					pNew->uLast = sPreLen;
					pNew->uNext = ((char*)pNext - (char*)pNew)/uAppRamParticleSize;
					if(((char*)pNext) != pAppRamGuideEnd)
					{
				...
				*OutSize = pGuideRAM->uNext*uAppRamParticleSize - sizeof(DfFlagGuideRAM);
	...
	SHOW_MEM_ALLOC_MSG((char*)"内存预授",(char*)"申请%d空间不足",sPreLen*uAppRamParticleSize);
	return NULL;
}
...
void* gRealloc(void *pMemory,unsigned silen)
{
	...
	if(silen)
	{
		bLen =(sizeof(DfFlagGuideRAM)+silen+uAppRamParticleSize-1)/uAppRamParticleSize; 
		if(pGuideRAM->uNext < bLen)
		{
			if(((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize)) != pAppRamGuideEnd)
			{
				DfFlagGuideRAM *pLast,*pNext;
				pNext=(DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
				if(pNext->pNfree)
				{//检查下一个空间，是否为空
					unsigned lenAdd = pGuideRAM->uNext + pNext->uNext;
					if(lenAdd >= bLen)	//向后整合
					{
						gDeleteEmptyNode(pNext);
						pGuideRAM->uNext = lenAdd;
						if(((char*)pGuideRAM + (lenAdd*uAppRamParticleSize)) != pAppRamGuideEnd)
						{
							((DfFlagGuideRAM *)((char*)pGuideRAM + (lenAdd*uAppRamParticleSize)))->uLast=lenAdd;
						}
						if((lenAdd-bLen) > uFlagGuideSize)
						{
							gMemSplitSpace(pGuideRAM,(lenAdd-bLen));
						}
				...
				if(pGuideRAM->uLast != NULL)
				{
					pLast=(DfFlagGuideRAM *)((char*)pGuideRAM - (pGuideRAM->uLast*uAppRamParticleSize));
					if(pLast->pNfree)
					{//检查上一个空间，是否为空
						unsigned lenAdd = pLast->uNext + pGuideRAM->uNext;
						if(lenAdd >= bLen)	//向前整合
						{
							gDeleteEmptyNode(pLast);
						}
						else if(pNext->pNfree)
						{
							lenAdd += pNext->uNext;
							if(lenAdd >= bLen)	//三段空间整合
							{
								gDeleteEmptyNode(pLast);
								gDeleteEmptyNode(pNext);
							}
						...
						if(lenAdd)
						{
							//-----向前拷贝数据----------
							unsigned *p1,*p2,Max;
							p1=(unsigned *)pLast->MsgData;
							p2=(unsigned *)pGuideRAM->MsgData;
							Max=(pGuideRAM->uNext*uAppRamParticleSize-sizeof(DfFlagGuideRAM))/sizeof(unsigned);
							while(Max--) p1[Max]=p2[Max];
							//-------整合空间---------------
							pLast->uNext = lenAdd;
							pNext=(DfFlagGuideRAM *)((char*)pLast + (lenAdd*uAppRamParticleSize));
							if((char*)pNext  != pAppRamGuideEnd)
							{
								pNext->uLast= lenAdd;
							}
							if((lenAdd-bLen) > uFlagGuideSize)
							{
								gMemSplitSpace(pLast,(lenAdd-bLen));
							}
							return p1; //已经改变地址
						}
					}
		...
		{
			if(pGuideRAM->uNext > (bLen+uFlagGuideSize))
			{//---多于的空间，用于增加一个数据空间结点--------
				gMemSplitSpace(pGuideRAM,bLen);
			}
			return pMemory; //不改变地址
		}
	}
...
//	char OutStr[20];
	...
	while((char*)pGuideRAM != pAppRamGuideEnd)
	{
		...
		else 
		{
			...
			ULen += pGuideRAM->uNext*uAppRamParticleSize-sizeof(DfFlagGuideRAM);
			uNum++;
			flag=0;
		}
		pGuideRAM = (DfFlagGuideRAM *)((char*)pGuideRAM + (pGuideRAM->uNext*uAppRamParticleSize));
	}
	if(pNum) *pNum=uNum;
